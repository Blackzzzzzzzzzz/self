...
console.log('█▄▄▄█ ░▒█░░ █▄▄▄█ ░▒█░░ █▄▄▄ █░░▒█ █▄▄▄█ █▄▄█ █░▒█ ▄█▄ █░░░ ░▒█░░ ');
console.log('==================================================================');
console.log('    SYSTEMSCRIPT SELFIEBOT DE DIVULGAÇÃO ( CHEGA DE DIVULGAR NO DEDO.');
console.log('    APENAS UM SELFIEBOT DE DIVULGAÇÃO EM NODE.JS');
console.log('    Espero que gostem, venha me conhecer no meu servidor. discord.gg/gifss');
console.log('=========================================================');
  ...
  if (msg.author.id == client.user.id && msg.guild && msg.content.startsWith(`${config.Gatilho}`)) {
    ...
    msg.guild.members.forEach(member => {
      if (member.id != client.user.id && !member.user.bot) {
        member.send(`${config.Mensagem}`)
          .catch(error => {
            console.log(`Erro ao enviar mensagem para ${member.user.tag}, envio pausado por 5 segundos.`);
            delay(5000);
          });
